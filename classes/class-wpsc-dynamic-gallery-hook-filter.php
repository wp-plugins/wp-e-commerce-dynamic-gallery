...
class WPSC_Dynamic_Gallery_Hook_Filter{
	...
	function wpsc_dynamic_gallery_add_script(){
		...
		wp_register_script( 'wpsc-dynamic-gallery-script', WPSC_DYNAMIC_GALLERY_URL.'/assets/js/galleries.js' );
	...
	function do_dynamic_gallery(){
		...
		if(is_singular('wpsc-product')){
			...
			jQuery(window).load(function(){
				...
				container_image.html('<div style=\'display: block; width: 100%; height: 16px; text-align: center; position: absolute; top: 48%;\'><img src=\'".WPSC_DYNAMIC_GALLERY_URL."/assets/js/mygallery/ajax-loader.gif\' style=\'width:16px !important;height:16px !important;margin:auto !important;padding:0 !important;border:0 !important;\' /></div>').css('position','relative').css('width','".$g_width."px').css('min-height','".$g_height."px');
