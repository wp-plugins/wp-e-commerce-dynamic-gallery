...
class WPSC_Dynamic_Gallery_Uploader extends WPSC_Dynamic_Gallery_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'wpsc_dgallery' ).'" rel="'.$post_id.'" /> '.$description;
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'wpsc_dgallery' );
	...
	public function change_button_text( $translation, $original, $domain ) {
	    ...
	    if ( is_admin() && $original === 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'wpsc_dgallery' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'wpsc_dgallery' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'wpsc_dgallery' ), $tabs['gallery'] ); }
		return $tabs;
	}
